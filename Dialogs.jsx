...
const Dialogs = ({ order, onClose }) => {
  if (!order) return null;

  // Use the order data instead of hardcoded items
  ...
  const getUrgencyColor = () => {
    const diffHours = (deadline - notedDate) / (1000 * 60 * 60);
  ...
  return (
    <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-md transform transition-all duration-300 max-h-[90vh] flex flex-col overflow-hidden">
        ...
        <div className="p-6 pb-2 border-b border-slate-50">
          ...
          <div className="flex flex-col text-sm">
            ...
            <span className={`font-semibold ${getUrgencyColor()}`}>Deadline: {formatDateTime(deadline)}</span>
          </div>
        </div>
        ...
        {showPicker && (
          <div className="absolute bottom-24 left-1/2 -translate-x-1/2 bg-white rounded-xl shadow-[0_20px_50px_rgba(0,0,0,0.3)] border border-slate-200 p-4 z-20 animate-in fade-in zoom-in-95 slide-in-from-bottom-4 duration-200 w-fit">
        ...
        <div className="p-5 border-t border-slate-100 bg-white flex gap-3">
          ...
          <button className="flex-1 bg-slate-900 text-white py-3.5 rounded-xl font-bold text-[10px] uppercase tracking-widest flex items-center justify-center gap-2 hover:bg-slate-800 transition-all shadow-lg shadow-slate-200">
