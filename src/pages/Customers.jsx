...
import { 
  ...
  Upload, Check, X, ChevronDown, Eye, EyeOff, AlertTriangle
} from 'lucide-react';
...
export default function CustomersPage({ setHeaderActions }) {
  ...
  const [newCustomer, setNewCustomer] = useState({ name: '', area: '' });
    ...
    setHeaderActions(
      <div className="flex gap-2">
        ...
        <button onClick={() => setIsAdding(true)} className="px-3 py-1.5 text-xs font-bold bg-emerald-600 text-white rounded-lg hover:bg-emerald-700 flex items-center gap-2">
          <PlusCircle className="h-3.5 w-3.5" /> Add Customer
        </button>
      </div>
    );
    ...
    <div className="space-y-4 pb-20">
      <div className="flex gap-2 relative" ref={settingsRef}>
        <div className="relative flex-1">
          ...
          <input type="text" placeholder="Search customers..." className="w-full pl-9 pr-4 py-2 text-sm border border-slate-200 rounded-xl outline-none" 
                 value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
        </div>
        {/* FIX: Clicking button while open now closes the settings menu */}
        <button 
          onClick={() => setShowSettings(!showSettings)} 
          className={`p-2 border rounded-xl transition-all ${showSettings ? 'bg-slate-900 text-white border-slate-900' : 'bg-white text-slate-600 border-slate-200'}`}
        >
              ...
              {columns.map(col => (
                <div key={col.id} className="flex items-center justify-between text-xs py-1.5 px-2 hover:bg-slate-50 rounded-lg group">
                  ...
                  <div className="flex items-center gap-2">
                      <button 
                        onClick={() => setColumns(columns.map(c => c.id === col.id ? {...c, visible: !c.visible} : c))} 
          ...
          <table className="w-full text-left border-collapse table-fixed min-w-max">
            ...
            <tbody className="divide-y divide-slate-50">
              {filteredCustomers.map((c) => (
                <tr key={c.id} onClick={() => setSelectedDetails(c)} className="cursor-pointer hover:bg-slate-50/50 transition-colors">
                  {visibleColumns.map(col => (
                    <td key={col.id} className="p-3 text-slate-800 text-sm truncate font-medium">{c[col.id] || '-'}</td>
                  ))}
                </tr>
              ))}
            </tbody>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm" onClick={() => !isEditingDetails && !confirmDelete && setSelectedDetails(null)}>
          <div className="bg-white w-full max-w-sm rounded-2xl shadow-2xl overflow-hidden" onClick={e => e.stopPropagation()}>
            <div className="px-4 py-3 bg-slate-50 border-b border-slate-100 flex justify-between items-center">
              <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{isEditingDetails ? 'Editing' : 'Profile'}</span>
                ...
                <tbody>
                  {columns.map(col => (
                    <tr key={col.id} className="border-b border-slate-50 last:border-0">
                      ...
                      <td className="p-3">
                        {isEditingDetails ? (
                          <input type="text" className="w-full p-1 border border-slate-200 rounded text-sm font-medium outline-none focus:border-emerald-500"
                            value={selectedDetails[col.id] || ''} onChange={(e) => setSelectedDetails({...selectedDetails, [col.id]: e.target.value})} />
                        ) : (
                          <span className="text-sm font-medium text-slate-700">{selectedDetails[col.id] || '-'}</span>
                        )}
                      </td>
                    </tr>
                  ))}
            ...
            <div className="p-3 bg-slate-50 border-t border-slate-100 space-y-2">
              {confirmDelete ? (
                <div className="bg-rose-50 p-3 rounded-xl border border-rose-100 flex flex-col gap-2">
                  <div className="flex items-center gap-2 text-rose-700 font-bold text-xs"><AlertTriangle className="h-4 w-4" /> Remove customer?</div>
                  <div className="flex gap-2">
                    <button onClick={() => { deleteCustomer(selectedDetails.id); setSelectedDetails(null); setConfirmDelete(false); }} className="flex-1 py-1.5 bg-rose-600 text-white rounded-lg text-xs font-bold">Yes, Delete</button>
                ...
                <div className="flex gap-2">
                  ...
                  <button onClick={() => { updateCustomer(selectedDetails.id, selectedDetails); setIsEditingDetails(false); }} className="flex-1 py-2 bg-emerald-600 text-white font-bold rounded-xl text-xs uppercase shadow-lg shadow-emerald-100">Save</button>
                </div>
              ) : (
                <div className="flex gap-2">
      ...
      {isImporting && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm">
          <div className="bg-white w-full max-w-xs rounded-2xl p-5 space-y-4 shadow-2xl text-center">
            <h3 className="font-black text-slate-800 text-sm uppercase tracking-wider">Import Customers</h3>
            <div className="p-3 bg-slate-50 border border-slate-100 rounded-xl space-y-1 text-left">
              <p className="text-[10px] font-bold text-slate-400 uppercase">Supported Headers: name, area</p>
            </div>
            <div className="flex flex-col gap-2">
              {/* MOBILE FIX: accept attribute expanded for wider device support */}
              <input 
                type="file" 
                id="import-file" 
                className="hidden" 
      ...
      {isAdding && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/30 backdrop-blur-sm">
          <div className="bg-white w-full max-w-xs rounded-3xl p-5 space-y-3 shadow-2xl">
            <h3 className="text-xs font-black text-slate-800 uppercase text-center mb-1">New Customer</h3>
            <input type="text" placeholder="Name" className="w-full p-3 bg-slate-50 border border-slate-100 rounded-xl outline-none text-sm font-medium" 
                   value={newCustomer.name} onChange={(e) => setNewCustomer({...newCustomer, name: e.target.value})} />
            <input type="text" placeholder="Area" className="w-full p-3 bg-slate-50 border border-slate-100 rounded-xl outline-none text-sm font-medium" 
                   value={newCustomer.area} onChange={(e) => setNewCustomer({...newCustomer, area: e.target.value})} />
            <div className="flex gap-2 pt-2">
              <button onClick={() => setIsAdding(false)} className="flex-1 py-3 text-[10px] font-black text-slate-400 uppercase">Cancel</button>
              <button onClick={() => { if(newCustomer.name){ addCustomer(newCustomer); setIsAdding(false); setNewCustomer({name:'', area:''}); } }} 
